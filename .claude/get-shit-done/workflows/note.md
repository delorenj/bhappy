...
- **Global scope**: `/home/delorenj/code/bhappy/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/delorenj/code/bhappy/.claude/notes/`)
...
2. Glob `/home/delorenj/code/bhappy/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/delorenj/code/bhappy/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/delorenj/code/bhappy/.claude/notes/` — works in any directory
